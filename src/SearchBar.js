		...
		return (
			<>
				<form
					name="nav-search"
					className="omnibox"
					onSubmit={this.search_submit.bind(this)}
				>
					<input
						className="bar"
						...
						required
					></input>
					<datalist id="nav-omnibox" ref={this.fields}>
						{this.state.omnibox_entries}
					</datalist>
					...
					<button
						className="cancel"
						onClick={this.close_search.bind(this)}
						type="button"
					>
						<CancelSVG />
					</button>
				</form>
				<button className="search" onClick={this.open_search.bind(this)}>
					<SearchSVG />
				</button>
			</>
		);
