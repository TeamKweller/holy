	...
	componentDidMount() {
		{
			...
			const html = `<!DOCTYPE HTML><html><head><title>about:blank</title><meta charset='utf-8' /></head><body><script src=${JSON.stringify(
				cdn
			)}></script></body></html>`;
	...
	render() {
		...
		return (
			<main>
				...
				<p>
					{obfuscate(<>Insta-Proxy</>)} is a new script that allows you to open
					an {obfuscate(<>unblocked proxy</>)} when needed.
				</p>
				<h2>How to install {obfuscate(<>Insta-Proxy</>)}:</h2>
				<p>
					To install {obfuscate(<>Insta-Proxy Addon</>)}, simply download the
					file below: <br />
				...
				<ol>
					...
					<li>
						Look for {obfuscate(<>Insta-Proxy.html</>)} (If you can't find{' '}
						{obfuscate(<>Insta-Proxy.html</>)}, download it from above)
					</li>
					<li>Open {obfuscate(<>Insta-Proxy.html</>)} in your web browser.</li>
				</ol>
				<p>
					Once you have completed all the steps, you will be on{' '}
					{obfuscate(<>Insta-Proxy</>)}.
				</p>
			</main>
		);
	}
